package com.nanum.board.utils.s3;
...
public class S3UploaderService {
    ...
    public S3UploadDto uploadBoards(MultipartFile multipartFiles, String bucket, String dirName) throws IOException {
        File uploadFile = convert(multipartFiles)  // 파일 변환할 수 없으면 에러
                .orElseThrow(() -> new IllegalArgumentException("error: MultipartFile -> File convert fail"));
        String originalFilename = multipartFiles.getOriginalFilename();
        return uploads(uploadFile, bucket, dirName, originalFilename);
    }

    public S3UploadDto uploads(File uploadFile, String bucket, String dirName, String originalFilename) throws IOException {
        String fileName = dirName + "/" + UUID.randomUUID() + uploadFile.getName();   // S3에 저장된 파일 이름
        String uploadImageUrl = putS3(uploadFile, bucket, fileName); // s3로 업로드
        removeNewFile(uploadFile);
        return S3UploadDto.builder()
